...
var WYSIWYG = {
	...
	Settings: function() {
		...
		// Images Directory
		this.ImagesDir = "js_libs/openwysiwyg/images/";
		
		// Popups Directory
		this.PopupsDir = "js_libs/openwysiwyg/popups/";
		
		// CSS Directory File
		this.CSSFile = "js_libs/openwysiwyg/styles/wysiwyg.css";		
		...
		this.RemoveFormatConfMessage = "Clean HTML inserted by Microsoft Word ?";
		
		// Nofication if browser is not supported by openWYSIWYG, leave it blank for no message output.
		this.NoValidBrowserMessage = "The Rich Text editor does not support your browser.";
	...
	_generate: function(n, settings) {
		...
		if($Q("textMode" + n)) {
			$Q("textMode" + n).style.display = 'none'; 
		}
	...
	updateStatusBar: function(n) {
		...
		// update the status bar 	
		var statusbar = $Q("wysiwyg_statusbar_" + n);
		if(statusbar){ 
	...
	writeDropDown: function(n, id) {
		...
		output += '<td onMouseOver="$Q(\'img_' + dropdown.id + '_' + n + '\').src=\'' + imageOn + '\';" onMouseOut="$Q(\'img_' + dropdown.id + '_' + n + '\').src=\'' + image + '\';">';
	...
	closeDropDowns: function(n, exid) {
		...
		for(var id in dropdowns) {
			...
			if(dropdown.id != exid) {
				...
				if($Q(divId)) $Q(divId).style.display = 'none';
			}
		}			
	},
	...
	openDropDown: function(n, id) {
		...
		if($Q(divId).style.display == "none") {	
			$Q(divId).style.display = "block"; 
		}
		else {
			$Q(divId).style.display = "none"; 
		}
		$Q(divId).style.position = "absolute";
	...
	viewSource: function(n) {
		...
		if($Q('HTMLMode' + n)) {
		    $Q('HTMLMode' + n).style.display = 'none'; 
		}
	    if($Q('textMode' + n)) {
		    $Q('textMode' + n).style.display = 'block';
		}
	...
	viewText: function(n) { 
		...
		if($Q('HTMLMode' + n)) {
			$Q('HTMLMode' + n).style.display = 'block';
		}
